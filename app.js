...
mongoose.connect("mongodb+srv://admin-anuj:Test123@cluster0.n7rqr.mongodb.net/todolistDB?retryWrites=true&w=majority", { useUnifiedTopology: true, useNewUrlParser: true });
...
app.get("/", function (req, res) {
  ...
  Item.find({}, function (err, foundItems) {
    if (foundItems == 0) {
      ...
      res.render("index.ejs", { listTitle: date.getDate(), items: foundItems, ls: lists });
    }
  });
});
    ...
    if (!err) {
      if (!foundlist) {
        ...
        List.find({}, function (err, list) {
          if (!err) {
            lists = list;
            res.render("index.ejs", { listTitle: customListName, items: foundlist.items, ls: lists });
          } 
        });
